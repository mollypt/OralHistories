...
def clean_text(filepath, filename):
    with open(filepath, 'r', encoding='utf-8') as file:
        ...
        # cut off metadata
        s = interview.index(">")
        interview = interview[s+1:]

        result = []
        in_seg=False
        start_tag=False
        for i, c in enumerate(interview):
            if c == '\n' and i<len(interview)-1:
                if '\n' in interview[i + 1:] and ':' in interview[i + 1:]:
                    if interview[i+1:].index('\n') < interview[i+1:].index(':'):
                        result.append(" ")
                    else:
                        result.append(c)
                elif ':' not in interview[i + 1:]:
                    result.append(" ")
                else:
                    result.append(c)
            elif (c == '<' and interview[i+1] == "E") or in_seg:
                in_seg=True
                if c == '<' and interview[i+1] == "B":
                    start_tag = True
                if c == '>' and start_tag:
                    in_seg=False
                    start_tag = False
    ...
    # Save the cleaned plain text to a file
    output_filename = f"transcript_lines/{filename[:-4]}.txt"
    with open(output_filename, 'w', encoding='utf-8', errors='ignore') as output_file:
        output_file.write(result)


def find_tags(filepath, filename):
    with open(filepath, 'r', encoding='utf-8') as file:
        interview = file.read()
        lines = interview.splitlines()
        for line in lines:
            if "copyright" in str(line).lower():
                i = line.lower().index("copyright")
                print(f"{filename}: {line[i:]}")
        # for line in lines:
        #     if "narr. note" in str(line).lower():
        #         i = line.lower().index("narr. note")
        #         print(f"{filename}: {line[i:]}")

#clean_text('transcripts_txt/ddr-densho-1000-81-transcript-9e2a615857.txt', 'ddr-densho-1000-81-transcript-9e2a615857.txt')

# Run to clean text files
# directory = 'transcripts_txt'
# for filename in os.listdir(directory):
#     file_path = os.path.join(directory, filename)
#     if os.path.isfile(file_path):
#         if filename != ".DS_Store":
#             clean_text(file_path, filename)
...
# Run to write html transcripts > txt transcripts
...
#html_to_text('transcripts/ddr-densho-1000-83-transcript-8859458391.htm', 'ddr-densho-1000-83-transcript-8859458391.txt')
